...
app.post("/login", async (req, res) => {
  try {
    const accountsQry = await pool.query("SELECT * FROM account;");
    const accountsRows = accountsQry.rows;
    const { passwordInput } = req.body;
    let foundAccount = accountsRows.find(
      (account) => account.password === passwordInput
    );

    let loginResult = {};
    if (foundAccount != undefined) {
      loginResult.resultMsg = "Login success!";
      ...
      loginResult.resultMsg = "Incorrect password.";
  ...
  try {
    ...
    entryTransactionsQry.rows.forEach((entryTransaction) => {
      ...
      if (currEntryIdx === -1) {
        ...
        currEntryIdx =
          dbEntriesWithTransactions.length > 0
            ? dbEntriesWithTransactions.length - 1
            : 0;
      }
...
app.get("/all-transactions", (req, res) =>
  res.json({
    transactions: tdAmeritradeData.filter(
...
app.post("/insert-entry", async (req, res) => {
  ...
  try {
    ...
    entryTransactionSqlInsertArr = entryDto.transactionDtos.map(
      (transactionDto) => [
        insertedEntry.journal_entry_id,
    ...
    res.status(500).send(
      JSON.stringify({
        resultMsg: `Failed to add journal entry due to server issue.`,
      })
    );
  }
});
...
app.put("/update-entry", async (req, res) => {
  ...
  try {
    ...
    entryDto.transactionDtos = entryDto.transactionDtos.filter(
      (transactionDto) => {
        ...
        if (
          entryTransactionsRows.findIndex(
            (entryTransactionsRow) =>
              entryTransactionsRow.transaction_id ==
              transactionDto.transactionId
          ) === -1
        ) {
    ...
    res.status(201).send({
      dbEntryWithTransactions: {
        ...updatedEntry,
        transactions: updatedEntryTransactionQry.rows,
      },
      resultMsg: `Updated journal entry with ID ${updatedEntry.journal_entry_id}`,
    });
  } catch (err) {
    res.status(500).send({
      resultMsg: `Failed to update journal entry with ID ${entryDto.entryId}`,
    });
  }
});
...
app.delete("/delete-entry", async (req, res) => {
  ...
  try {
    ...
    res.status(201).send(
      JSON.stringify({
        resultMsg: `Deleted journal entry with ID ${entryId}`,
      })
    );
  } catch (err) {
    res
      .status(500)
      .send({ resultMsg: `Failed to delete journal entry with ID ${entryId}` });
  }
});
