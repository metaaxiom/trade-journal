...
import "../styles/journal-entry-transactions-list.scss";

export default function JournalEntryTransactionsList(props) {
  const {
    entryDto,
    setEntryDto,
    showTransactionsControls = false,
    showPostDates = false,
  ...
  const toggleTransactionRemoval = (transactionId) => {
    ...
    setEntryDto((prevEntryDto) => {
      let updatedTransactionDtos = prevEntryDto.transactionDtos.map(
        (prevTransactionDto) => {
          let isMarkedForRemoval = prevTransactionDto.isMarkedForRemoval;
          ...
          if (!isMarkedForRemoval) {
            allTransactionsMarkedForRemovalNewVal = false;
          }
  ...
  return (
    <>
      <table
        className={`table--vertical entry__transactions-table ${additionalTableClasses}`}
      >
        <thead>
          <tr>
            ...
            <th
              className={`entry__transactions-table__heading--cost text--number text--data-${
                entryDto.transactionsCostSum >= 0 ? "green" : "red"
              }`}
            >
              {entryDto.transactionsCostSum
                ? Math.abs(entryDto.transactionsCostSum).toFixed(2)
                : ""}
            </th>
        ...
        <tbody>
          {Array.isArray(entryDto.transactionDtos) &&
            entryDto.transactionDtos
              .sort((transactionDtoPrev, transactionDtoCurr) =>
                transactionDtoPrev.ticker.localeCompare(
                  transactionDtoCurr.ticker
                )
              )
              .map((transactionDto, transactionDtoIdx) => (
                <tr
                  key={transactionDtoIdx}
                  className={
                    transactionDto.isMarkedForRemoval ? "to-remove" : ""
                  }
                >
                  ...
                  <td className="entry__transactions-table__cell--cost text--number">
                    <span
                      className={`text--data-${transactionDto.costTotalColor}`}
                    >
                  ...
                  <td
                    className={`entry__transactions-table__cell--instruction text--data-${transactionDto.instructionColor}`}
                  >
                    {transactionDto.instruction}
                  </td>
                  <td
                    className={`entry__transactions-table__cell--asset-subtype text--data-${transactionDto.assetSubTypeColor}`}
                  >
