...
import "../styles/journal-entry-list-modal.scss";
...
export default function JournalEntriesListModal(props) {
  ...
  const toggleFromListToEdit = (entryId) => {
    ...
    toggleEntryEditModal(
      { entryId: entryId, showGoBack: true },
      selectedEntryTransactionIds
  ...
  return (
    <Modal
      ...
      classes={"modal--entry-list"}
    >
      <p className="entry-list__text">
        Select an existing journal entry to add the selected transactions.
      </p>
        ...
        <table className="table--vertical entry-list__table">
          ...
          <tbody>
            {Array.isArray(entriesWithTransactions) &&
              entriesWithTransactions.map((entryDto, entryDtoIdx) => (
                <tr
                  key={entryDtoIdx}
                  onClick={() => {
                    toggleFromListToEdit(entryDto.entryId);
                  }}
                >
                  ...
                  <td className="entry-list__table__cell--tickers">
                    {Array.isArray(entryDto.transactionDtos) &&
                      entryDto.transactionDtos.reduce(
                        (transactionDtoPrev, transactionDtoCurr) => {
                          if (
                            !transactionDtoPrev.ticker.includes(
                              transactionDtoCurr.ticker
                            )
                          ) {
                            return {
                              ticker:
                                transactionDtoPrev.ticker +
                                " " +
                                transactionDtoCurr.ticker,
                            };
                          }
                  ...
                  <td
                    className={`entry-list__table__cell--total text--number text--${
                  ...
                  <td className="entry-list__table__cell--description">
                    {entryDto.description != null &&
                    entryDto.description.length > 15
                      ? entryDto.description.substring(0, 18).trim() + "..."
