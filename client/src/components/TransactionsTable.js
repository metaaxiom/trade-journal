...
import "../styles/transactions-table.scss";
...
export default function TransactionsTable(props) {
  const {
    ...
    entryDto,
    setEntryDto,
    toggleEntryEditModal,
    toggleEntryListModal,
    showEntryListModal,
    setEntriesWithTransactions,
  } = props;
  const [selectedEntryTransactionIds, setSelectedEntryTransactionIds] =
    React.useState([]);
  ...
  const toggleSelected = (transactionId) => {
    console.log(
      "attempting to add to (or remove from) staging:",
      transactionId
    );

    if (
      selectedEntryTransactionIds.findIndex(
        (selectedTransactionId) => selectedTransactionId == transactionId
      ) == -1
    ) {
      // if transaction id doesn't yet exist in the array, add it
      setSelectedEntryTransactionIds((prevSelectedIds) => [
        ...prevSelectedIds,
        transactionId,
      ]);
      return;
  ...
  const isAllSelected = () => {
    return (
      allTransactionDtos.length > 0 &&
      allTransactionDtos.length === selectedEntryTransactionIds.length
    );
  };
  ...
  const transactionSorts = {
    ...
    default: {
      ascending: (prevTransaction, currTransaction) =>
        prevTransaction[transactionSortCol] >
        currTransaction[transactionSortCol]
          ? -1
          : 1,
  ...
  const handleTransactionChangeSort = (sortCol) => {
    ...
    let sortOrder =
      transactionSortOrder === "ascending" ? "descending" : "ascending";
    // re-sort the same column in opposite order
    // also check that sort for col has the sortOrder
    if (
      sortCol === transactionSortCol &&
      (transactionSorts[sortCol] == undefined ||
        transactionSorts[sortCol][sortOrder] != undefined)
    ) {
  ...
  const getTransactionSort = () => {
    console.log(
      "transactionSorts[transactionSortCol]",
      transactionSorts[transactionSortCol]
    );
    let transactionSortsKey =
      transactionSorts[transactionSortCol] != undefined
        ? transactionSortCol
        : "default";
    console.log(
      "transactionSorts[transactionSortsKey][transactionSortOrder]",
      transactionSortsKey,
  ...
  const insertEntry = async () => {
    ...
    let intermInsertedEntryWithTransactions = toIntermediateDtoSyntax(
      response.data.dbEntryWithTransactions
    );
    let insertedTransactionDtos =
      intermInsertedEntryWithTransactions.transactions.map(
        (intermTransaction) => ({
          ...allTransactionDtos.find(
            (transactionDto) =>
              transactionDto.transactionId == intermTransaction.transactionId
          ),
          ...intermTransaction,
        })
      );
  ...
  useEffect(() => {
    document.title = "Trade Journal - Transactions";
  }, []);
    ...
    <>
      <div id="transactions-table_top-controls">
        <button
          ...
          disabled={selectedEntryTransactionIds.length == 0}
        >
          New Entry
        </button>
        <button
          onClick={toggleEntryListModal}
          className="btn btn--green"
          disabled={selectedEntryTransactionIds.length == 0}
        >
          Existing Entry
          ...
          <thead>
            <tr>
              ...
              <th scope="col" id="transactions-table__heading-transaction-date">
                <button
                  onClick={() => {
                    handleTransactionChangeSort("transactionDate");
                  }}
                >
              ...
              <th scope="col" id="transactions-table__heading-ticker">
                <button
                  onClick={() => {
                    handleTransactionChangeSort("ticker");
                  }}
                >
                  <i className="fa-solid fa-sort"></i>
                  Ticker
              ...
              <th scope="col" id="transactions-table__heading-instruction">
                <button
                  onClick={() => {
                    handleTransactionChangeSort("instruction");
                  }}
                >
                  <i className="fa-solid fa-sort"></i>
                  Side
              ...
              <th scope="col" id="transactions-table__heading-asset-subtype">
                <button
                  onClick={() => {
                    handleTransactionChangeSort("assetSubType");
                  }}
                >
                  <i className="fa-solid fa-sort"></i>
                  Type
              ...
              <th scope="col" id="transactions-table__heading-strike-price">
                <button
                  onClick={() => {
                    handleTransactionChangeSort("strikePrice");
                  }}
                >
                  <i className="fa-solid fa-sort"></i>
                  Strike
              ...
              <th scope="col" id="transactions-table__heading-exp-date">
                <button
                  onClick={() => {
                    handleTransactionChangeSort("expDate");
                  }}
                >
                  <i className="fa-solid fa-sort"></i>
                  Expiry
              ...
              <th scope="col" id="transactions-table__heading-qty">
                <button
                  onClick={() => {
                    handleTransactionChangeSort("qty");
                  }}
                >
                  <i className="fa-solid fa-sort"></i>
                  Qty
              ...
              <th scope="col" id="transactions-table__heading-cost-per-unit">
                <button
                  onClick={() => {
                    handleTransactionChangeSort("costPerUnit");
                  }}
                >
                  <i className="fa-solid fa-sort"></i>
                  Unit Cost
              ...
              <th scope="col" id="transactions-table__heading-cost-total">
                <button
                  onClick={() => {
                    handleTransactionChangeSort("costTotal");
                  }}
                >
                  <i className="fa-solid fa-sort"></i>
                  Premium
                ...
                <tr key={transactionDto.transactionId}>
                  <td className="transactions-table__cell-all-checkbox">
                    <input
                      ...
                      value={getTransactionCheckboxValue(
                        transactionDto.transactionId
                      )}
                      checked={getTransactionCheckboxValue(
                        transactionDto.transactionId
                      )}
                      onChange={() => {
                        toggleSelected(transactionDto.transactionId);
                      }}
                    />
                  </td>
                  ...
                  <td className="transactions-table__cell-ticker">
                    {transactionDto.ticker}
                  </td>
                  <td
                    className={`transactions-table__cell-instruction text--data-${transactionDto.instructionColor}`}
                  >
                    {transactionDto.instruction}
                  </td>
                  <td
                    className={`transactions-table__cell-asset-subtype text--data-${transactionDto.assetSubTypeColor}`}
                  >
                    {transactionDto.assetSubType}
                  </td>
                  <td className="transactions-table__cell-strike-price text--number">
                    {transactionDto.strikePrice != ""
                      ? parseInt(transactionDto.strikePrice).toFixed(2)
                      : ""}
                  </td>
                  <td className="transactions-table__cell-exp-date">
                    {buildDateTimeStr(transactionDto.expDate, false)}
                  </td>
                  <td className="transactions-table__cell-qty">
                    {transactionDto.qty}
                  </td>
                  <td
                    className={`transactions-table__cell-cost-per-unit text--number text--data-${transactionDto.costTotalColor}`}
                  >
                    {transactionDto.costPerUnitAbs.toFixed(2)}
                  </td>
                  <td
                    className={`transactions-table__cell-cost-total text--number text--data-${transactionDto.costTotalColor}`}
                  >
      ...
      <JournalEntryListModal
        ...
        selectedEntryTransactionIds={selectedEntryTransactionIds}
      ></JournalEntryListModal>
    </>
  );
}
